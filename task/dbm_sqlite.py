...
def get_db_manager_sqlite(database, duplicate_handling, transaction_size, cumulus_filter=None, auto_batching=True):
    global DB_SQLITE
    db_init_kwargs = {
        'database': database,
        'timeout': 900,
    ...
    return DBManagerSqlite(
        GranuleSQLite, DB_SQLITE, auto_batching, transaction_size, duplicate_handling, cumulus_filter
    )
...
class DBManagerSqlite(DBManagerPeewee):
    def __init__(self, model_class, database, auto_batching, transaction_size, duplicate_handling, cumulus_filter):
        ...
        super().__init__(
            ...
            VAR_LIMIT_SQLITE, EXCLUDED, chunked
        )
