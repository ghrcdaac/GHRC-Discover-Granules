...
set -o nounset
set -o pipefail
export REPO_NAME="gdg"
export SERVICE_NAME=GDG
...
function stop_ecs_task() {
  task_arns=$(aws ecs list-tasks --cluster $1-monolith-cluster --family $1-$2 --query "taskArns[*]" --region $AWS_REGION | tr -d '"[],')
  for task in $task_arns
  do
    aws ecs stop-task --cluster $1-monolith-cluster --task $task --region $AWS_REGION
  done
}
